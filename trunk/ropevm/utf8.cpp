...
void convertUtf8(const char* utf8, unsigned short *buff) {
    while(*utf8)
...
int utf8Comp(const char* ptr, const char* ptr2) {
    while(*ptr && *ptr2) {
        unsigned short c, c2;
...
char *findHashedUtf8(const char* string, int add_if_absent) {
    char *interned;
    char* string1 = const_cast<char*>(string);

    /* Add if absent, no scavenge, locked */
    findHashEntry(hash_table, string1, interned, add_if_absent, FALSE, TRUE, char*);

    return interned;
}

char *copyUtf8(const char* string) {
    char *buff = (char*)strcpy((char*)sysMalloc(strlen(string) + 1), string);
    const char *found = findHashedUtf8(buff, TRUE);
...
char *slash2dots2buff(const char *utf8, char *buff, int buff_len) {
    char *pntr = buff;
