...
int initJNILrefs() {
    ...
    if(frame != NULL) {
        ...
        if (frame->lrefs == 0) {
            frame->lrefs = new std::vector<Object*>;
        }
        return TRUE;
    }
...
JNIFrame *expandJNILrefs(JNIFrame *frame, int incr) {
...
//         }
    assert(frame->lrefs);
    std::vector<Object*>::iterator i = std::find(frame->lrefs->begin(), frame->lrefs->end(), ref);
    if (i != frame->lrefs->end())
        frame->lrefs->erase(i);
}
...
//     new_frame->ostack = (uintptr_t*)(new_frame + 1);
    ...
    new_frame->lvars[0] = (uintptr_t)frame; // save it
...
jobject Jam_NewDirectByteBuffer(JNIEnv *env, void *addr, jlong capacity) {
    ...
    if((buff = allocObject(buffImpl_class)) != NULL &&
        ...
        DummyFrame dummy;
        executeMethod(&dummy, buff, buffImpl_init_mb, NULL, rawdata, (int)capacity,
                      (int)capacity, 0);
    }
...
jobject Jam_PopLocalFrame(JNIEnv *env, jobject result) {
    popJNILrefFrame();
    return addJNILref((Object*)result);
}

jobject Jam_NewLocalRef(JNIEnv *env, jobject obj) {
    return addJNILref((Object*)obj);
}
...
jclass Jam_FindClass(JNIEnv *env, const char *name)
{
    ...
    if(last->prev) {
        ClassBlock *cb = CLASS_CB(last->mb->classobj);
...
Object *allocObjectClassCheck(Class *classobj) {
    ...
    if(initClass(classobj) == NULL)
        return NULL;

    return allocObject(classobj);
}
...
jclass Jam_GetObjectClass(JNIEnv *env, jobject obj) {
    return (jobject)((Object*)obj)->classobj;
}

jboolean Jam_IsInstanceOf(JNIEnv *env, jobject obj, jclass clazz) {
    return (obj == NULL) || isInstanceOf((Class*)clazz, ((Object*)obj)->classobj);
}

jmethodID getMethodID(JNIEnv *env, jclass clazz, const char *name, const char *sig, char is_static) {
    Class *classobj = initClass((Class *)clazz);
    MethodBlock *mb = NULL;

    if(classobj != NULL) {
        if(!IS_PRIMITIVE(CLASS_CB(classobj))) {
            ...
            if(method_name != NULL && method_sig != NULL) {
                if(method_name == SYMBOL(object_init) ||
                   method_name == SYMBOL(class_init))
                    mb = findMethod(classobj, method_name, method_sig);
                else
                    mb = lookupMethod(classobj, method_name, method_sig);
            }
        }
...
jfieldID Jam_GetFieldID(JNIEnv *env, jclass clazz, const char *name, const char *sig) {
    ...
    if(classobj != NULL) {
        if(field_name != NULL && field_sig != NULL)
            fb = lookupField(classobj, field_name, field_sig);
...
jfieldID Jam_GetStaticFieldID(JNIEnv *env, jclass clazz, const char *name, const char *sig) {
    ...
    if(classobj != NULL) {
        if(field_name != NULL && field_sig != NULL)
            fb = findField(classobj, field_name, field_sig);
...
jobject Jam_NewObject(JNIEnv *env, jclass clazz, jmethodID methodID, ...) {
    ...
    if(ob) {
        ...
        DummyFrame dummy;
        executeMethodVaList(&dummy, ob, ob->classobj, (MethodBlock*)methodID, jargs);
        va_end(jargs);
    }
...
jobject Jam_NewObjectA(JNIEnv *env, jclass clazz, jmethodID methodID, jvalue *args) {
    ...
    if(ob) {
        DummyFrame dummy;
        executeMethodList(&dummy, ob, ob->classobj, (MethodBlock*)methodID, (u8*)args);
    }
    return (jobject) addJNILref(ob);
}
...
jobject Jam_NewObjectV(JNIEnv *env, jclass clazz, jmethodID methodID, va_list args) {
    ...
    if(ob) {
        DummyFrame dummy;
        executeMethodVaList(&dummy, ob, ob->classobj, (MethodBlock*)methodID, args);
    }
    return (jobject) addJNILref(ob);
}

jarray Jam_NewObjectArray(JNIEnv *env, jsize length, jclass elementClass, jobject initialElement) {
    const char *element_name = CLASS_CB((Class*)elementClass)->name;
    ...
    if(array_class != NULL) {
        ...
        if(array != NULL) {
            if(initialElement != NULL) {
                Object **data = (Object**)ARRAY_DATA(array);
...
extern struct _JNIInvokeInterface Jam_JNIInvokeInterface;
...
native_type Jam_Call##type##Method(JNIEnv *env, jobject obj, jmethodID mID, ...) { \
    ...
    va_start(jargs, mID);                                               \
    DummyFrame dummy;                                                   \
    ret = (native_type*) executeMethodVaList(&dummy, ob, ob->classobj, mb, jargs); \
    va_end(jargs);                                                      \
                                                                        \
    return *ret;                                                        \
}                                                                       \
...
native_type Jam_Call##type##MethodV(JNIEnv *env, jobject obj, jmethodID mID, va_list jargs) { \
    ...
    return mb == NULL ? (native_type)0 :                                \
                       *(native_type*)executeMethodVaList(&dummy, ob, ob->classobj, mb, jargs); \
}                                                                       \
                                                                                                 \
native_type Jam_Call##type##MethodA(JNIEnv *env, jobject obj, jmethodID mID, jvalue *jargs) { \
    ...
    DummyFrame dummy;                                                   \
    return mb == NULL ? (native_type)0 :                                \
        *(native_type*)executeMethodList(&dummy, ob, ob->classobj, mb, (u8*)jargs); \
}
...
native_type Jam_Call##type##Method(JNIEnv *env, jobject obj, jmethodID mID, ...) { \
    ...
    va_start(jargs, mID);                                               \
    DummyFrame dummy;                                                   \
    ret = (uintptr_t *)executeMethodVaList(&dummy, ob, ob->classobj, mb, jargs); \
    va_end(jargs);                                                      \
                                                                        \
...
native_type Jam_Call##type##MethodV(JNIEnv *env, jobject obj, jmethodID mID, va_list jargs) { \
    ...
    DummyFrame dummy;                                                   \
    return mb == NULL ? (native_type)0 :                                \
                        (native_type)*(uintptr_t*)executeMethodVaList(&dummy, ob, ob->classobj, mb, jargs); \
}                                                                       \
                                                                        \
native_type Jam_Call##type##MethodA(JNIEnv *env, jobject obj, jmethodID mID, jvalue *jargs) { \
    Object *ob = (Object *)obj;                                         \
    ...
    executeMethodList(&dummy, ob, ob->classobj, mb, (u8*)jargs);        \
}
...
native_type Jam_CallNonvirtual##type##Method(JNIEnv *env, jobject obj, jclass clazz, \
    ...
    DummyFrame dummy;                                                   \
    ret = (native_type*)                                                \
              executeMethodVaList(&dummy, (Object*)obj, (Class*)clazz, (MethodBlock*)methodID, jargs); \
    va_end(jargs);                                                      \
                                                                        \
    return *ret;                                                        \
}                                                                       \
...
native_type Jam_CallNonvirtual##type##MethodA(JNIEnv *env, jobject obj, jclass clazz, \
    ...
    DummyFrame dummy;                                                   \
    return *(native_type*)                                              \
            executeMethodList(&dummy, (Object*)obj, (Class*)clazz, (MethodBlock*)methodID, (u8*)jargs); \
}
...
native_type Jam_CallNonvirtual##type##Method(JNIEnv *env, jobject obj, jclass clazz, \
    ...
    va_start(jargs, methodID);                                          \
    DummyFrame dummy;                                                   \
    ret = (uintptr_t*)executeMethodVaList(&dummy, (Object*)obj, (Class*)clazz, (MethodBlock*)methodID, jargs); \
    va_end(jargs);                                                      \
                                                                        \
    return (native_type)*ret;                                           \
}                                                                       \
...
native_type Jam_CallStatic##type##Method(JNIEnv *env, jclass clazz,     \
    ...
    va_start(jargs, methodID);                                          \
    DummyFrame dummy;                                                   \
    ret = (native_type*) executeMethodVaList(&dummy, NULL, (Class*)clazz, (MethodBlock*)methodID, jargs); \
    va_end(jargs);                                                      \
                                                                        \
    return *ret;                                                        \
}                                                                       \
...
native_type Jam_CallStatic##type##MethodV(JNIEnv *env, jclass clazz, jmethodID methodID, \
    ...
    DummyFrame dummy;                                                   \
    return *(native_type*)                                              \
            executeMethodVaList(&dummy, NULL, (Class*)clazz, (MethodBlock*)methodID, jargs); \
}                                                                       \
                                                                        \
native_type Jam_CallStatic##type##MethodA(JNIEnv *env, jclass clazz, jmethodID methodID, \
    ...
    DummyFrame dummy;                                                   \
    return *(native_type*)                                              \
            executeMethodList(&dummy, NULL, (Class*)clazz, (MethodBlock*)methodID, (u8*)jargs); \
}
...
native_type Jam_CallStatic##type##Method(JNIEnv *env, jclass clazz,     \
    ...
    va_start(jargs, methodID);                                          \
    DummyFrame dummy;                                                   \
    ret = (uintptr_t*)executeMethodVaList(&dummy, NULL, (Class*)clazz, (MethodBlock*)methodID, jargs); \
    va_end(jargs);                                                      \
                                                                        \
    return (native_type)*ret;                                           \
}                                                                       \
...
jobject Jam_CallObjectMethod(JNIEnv *env, jobject obj, jmethodID methodID, ...) {
    ...
    va_start(jargs, methodID);
    DummyFrame dummy;
    ret = addJNILref(*(Object**) executeMethodVaList(&dummy, ob, ob->classobj, mb, jargs));
    va_end(jargs);
    return (jobject)ret;
}
...
jobject Jam_CallObjectMethodV(JNIEnv *env, jobject obj, jmethodID methodID, va_list jargs) {
    ...
    return mb == NULL ? NULL : (jobject)addJNILref(*(Object**)
                                        executeMethodVaList(&dummy, ob, ob->classobj, mb, jargs));
}
...
jobject Jam_CallNonvirtualObjectMethod(JNIEnv *env, jobject obj, jclass clazz,
    ...
    va_start(jargs, methodID);
    DummyFrame dummy;
    ret = addJNILref(*(Object**) executeMethodVaList(&dummy, (Object*)obj,
...
jobject Jam_CallNonvirtualObjectMethodV(JNIEnv *env, jobject obj, jclass clazz,
                jmethodID methodID, va_list jargs) {
    DummyFrame dummy;
    return (jobject)addJNILref(*(Object**) executeMethodVaList(&dummy, (Object*)obj,
                            (Class*)clazz, (MethodBlock*)methodID, jargs));
}
...
jobject Jam_CallNonvirtualObjectMethodA(JNIEnv *env, jobject obj, jclass clazz,
                jmethodID methodID, jvalue *jargs) {
    DummyFrame dummy;
    return (jobject)addJNILref(*(Object**) executeMethodList(&dummy, (Object*)obj,
...
jobject Jam_CallStaticObjectMethod(JNIEnv *env, jclass clazz, jmethodID methodID, ...) {
    ...
    va_start(jargs, methodID);
    DummyFrame dummy;
    ret = addJNILref(*(Object**) executeMethodVaList(&dummy, NULL,
                              (Class*)clazz, (MethodBlock*)methodID, jargs));
    va_end(jargs);
    return (jobject)ret;
}
...
jobject Jam_CallStaticObjectMethodV(JNIEnv *env, jclass clazz,
                jmethodID methodID, va_list jargs) {
    DummyFrame dummy;
    return (jobject)addJNILref(*(Object**) executeMethodVaList(&dummy, NULL,
                              (Class*)clazz, (MethodBlock*)methodID, jargs));
}
...
jobject Jam_CallStaticObjectMethodA(JNIEnv *env, jclass clazz,
                jmethodID methodID, jvalue *jargs) {
    DummyFrame dummy;
    return (jobject)addJNILref(*(Object**) executeMethodList(&dummy, NULL,
                            (Class*)clazz, (MethodBlock*)methodID, (u8*)jargs));
}
...
static jint attachCurrentThread(void **penv, void *args, int is_daemon) {
    if(threadSelf() == NULL) {
        ...
        if(args != NULL) {
            ...
            group = (Object*)attach_args->group;
        }
...
jint parseInitOptions(JavaVMInitArgs *vm_args, InitArgs *args) {
    ...
    for(i = 0; i < vm_args->nOptions; i++) {
        ...
        if(strcmp(string, "vfprintf") == 0)
            args->vfprintf = (int (*)(FILE*, const char*, va_list))vm_args->options[i].extraInfo;

        else if(strcmp(string, "exit") == 0)
            args->exit = (void (*)(int))vm_args->options[i].extraInfo;

        else if(strcmp(string, "abort") == 0)
            args->abort = (void (*)())vm_args->options[i].extraInfo;
            ...
            char *pntr, *key = (char*)strcpy((char*)sysMalloc(strlen(string+2) + 1), string+2);
