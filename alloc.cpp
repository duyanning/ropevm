...
Thread::scan()
{
    ...
    markConservativeRoot(threadSelf()->get_current_core()->get_current_mode()->exception);
...
void threadClassData(Class *classobj, Class *new_addr) {
    ClassBlock *cb = CLASS_CB(classobj);
    ConstantPool *cp = &cb->constant_pool;
    FieldBlock *fb = cb->fields;
    int i;
    ...
    for(i = 0; i < cb->fields_count; i++)
        cb->fields[i].classobj = new_addr;

    for(i = 0; i < cb->methods_count; i++)
        cb->methods[i].classobj = new_addr;
}
...
out:
    ...
    threadReference((Object**)&ob->classobj);
...
Object *allocTypeArray(int type, int size) {
    ...
    return allocArray(classobj, size, el_size);
}
...
Object *allocMultiArray(Class *array_class, int dim, intptr_t *count) {
    ...
    int i;
    Object *array;
    const char *element_name = CLASS_CB(array_class)->name + 1;
...
Object *cloneObject(Object *ob)
{
    ...
    if(clone != NULL) {
        ...
        if(HDR_SPECIAL_OBJ(hdr)) {
            ...
            if(IS_CLASS_LOADER(CLASS_CB(clone->classobj)))
...
void *gcMemMalloc(int n) {
    ...
    int *mem = (int*)mmap(0, size, PROT_READ|PROT_WRITE,
                    MAP_PRIVATE|MAP_ANON, -1, 0);
...
void *sysMalloc(int size) {
    int n = size < (int)sizeof(void*) ? sizeof(void*) : size;
    void *mem = malloc(n);
